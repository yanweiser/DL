...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embeddings = nn.Parameter(torch.tensor(embeddings, device=my_device))
        ...
        self.embed_to_hidden_weight = nn.Parameter(data=nn.init.xavier_uniform_(torch.zeros((self.embed_size*self.n_features, self.hidden_size), device=my_device)))
        self.embed_to_hidden_bias = nn.Parameter(data=nn.init.uniform_(torch.zeros(self.hidden_size, device=my_device)))

        self.dropout_layer = nn.Dropout(p=self.dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(data=nn.init.xavier_uniform_(torch.zeros((self.hidden_size, self.n_classes), device=my_device)))
        self.hidden_to_logits_bias = nn.Parameter(data=nn.init.uniform_(torch.zeros(self.n_classes, device=my_device)))
    ...
    def forward(self, w):
        ...
        ### YOUR CODE HERE
        ### TODO:
        ###     Complete the forward computation. In addition, include a dropout layer
        ###     as decleared in `__init__` after the ReLU function.
        ###
        ### Note: We do not apply the softmax to the logits here, because
        ### the loss function (torch.nn.CrossEntropyLoss) applies it more efficiently.
        ###
        ### Check out the following docs for support:
        ###     Matrix product: https://pytorch.org/docs/stable/torch.html#torch.matmul
        ###     ReLU: https://pytorch.org/docs/stable/nn.html?highlight=relu#torch.nn.functional.relu

        
        embeds = self.embedding_lookup(w)
        h = self.ReLU(torch.matmul(embeds, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h_drop = self.dropout_layer(h)
