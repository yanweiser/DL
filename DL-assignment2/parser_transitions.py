...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        steps = model.predict(batch)
        for i in range(len(steps)):
            unfinished_parses[i].parse_step(steps[i])
        for u_parse in unfinished_parses:
            if not u_parse.buffer and len(u_parse.stack) == 1:
                unfinished_parses.remove(u_parse)
